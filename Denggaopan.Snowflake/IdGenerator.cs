...
namespace Denggaopan.Snowflake
{
    public class IdGenerator
    {
        ...
        public IdGenerator(long workerId, long dataCenterId, long sequence = 0L)
        {
            ...
            {
                throw new ArgumentException(string.Format("workerId 必须大于0，且不能大于MaxWorkerId： {0}", MaxWorkerId));
            }

            if (dataCenterId > MaxDataCenterId || dataCenterId < 0)
            {
                throw new ArgumentException(string.Format("dataCenterId 必须大于0，且不能大于MaxDataCenterId： {0}", MaxDataCenterId));
            }
